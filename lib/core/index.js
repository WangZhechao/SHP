var Promise = require('bluebird'),
	...
	path = require('path'),
	MysqlStore = require('../utils/connect-mysql.js')(session),
	config = require(path.resolve(__dirname, '../config')),
	utils = require(path.resolve(__dirname, '../utils')),
	system = require(path.resolve(__dirname, './system.js')),
	SHPServer = require(path.resolve(__dirname, './server.js'));
...
function init(options) {
	...
	return config.load(options.config).then(function(config) {
		...
		adminApp.use(session({
			...
			store: config.server.memoryStore ? null : new MysqlStore(),
			resave: true,
