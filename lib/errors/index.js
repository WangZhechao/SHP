var path = require('path'),
	...
	ValidationError = require(path.resolve(__dirname, './validation-error')),
	InternalServerError = require(path.resolve(__dirname, './internal-server-error')),
	NoPermissionError = require(path.resolve(__dirname, './no-permission-error')),
	errors;
...
errors = {
    ...
    renderErrorPage: function (err, req, res, next) {
        ...
        var self = this,
        	code = 500,
        	defaultErrorTemplatePath = path.resolve(errors.paths.views, 'error.hbs');
        ...
        function renderErrorInt(errorView) {
            ...
            res.status(code).render((errorView || 'error'), {
                ...
                return res.status(500).send(
                    ...
                    '<pre>' + utils.escapeExpression(templateErr.message || templateErr) + '</pre>' +
                    '<br ><p>whilst trying to render an error page for the error: </p>' +
                    code + ' ' + '<pre>'  + utils.escapeExpression(err.message || err) + '</pre>'
                );
            });
        }
